...
@endif
    ...
    <div class="max-w-6xl mx-auto mt-8 px-4">
        ...
        <!-- Table Container Wrapped to match -->
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($transactions as $transaction)
                        <tr class="bg-white hover:bg-gray-50 transition-colors">
                            <td class="p-4 font-semibold text-sm text-gray-900">{{ $transaction->no_inv }}</td>
                            <td class="p-4 text-sm text-gray-700">{{ $transaction->customer->nama_customer ?? 'Unknown Customer' }}</td>
                            <td class="p-4 text-sm font-medium text-gray-900">Rp. {{ number_format($transaction->total) }}</td>
                            <td class="p-4 text-sm text-gray-600">{{ $transaction->created_at->format('Y-m-d') }}</td>
                            <td class="p-4 text-center">
                                <div class="flex items-center justify-center space-x-3">
                                    <form action="{{ route('transactions.destroy', $transaction->id) }}" method="POST" onsubmit="return confirm('Delete this transaction? Stock will be restored.')" class="inline-flex items-center m-0 p-0">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="flex items-center justify-center text-gray-500 hover:text-red-600 transition-colors focus:outline-none" title="Delete Invoice">
                                    ...
                                    <a href="{{ route('transactions.edit', $transaction->id) }}" class="flex items-center justify-center text-gray-500 hover:text-blue-600 transition-colors" title="Edit Details">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="p-8 text-center text-sm text-gray-400 font-medium">No invoices found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
